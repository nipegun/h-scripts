...
The program is stored by content in `mcOSisoCreator/_/temp/bootstrap/runtime-<SHA256>/`. Repeated runs verify and reuse it; a new release uses another directory while preserving ISO outputs and Apple downloads. Do not edit that verified copy; use repository sources for development. If cached code is damaged, the launcher stops and identifies the file. A new `--directory` downloads a clean copy.
