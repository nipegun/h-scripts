...
```sh
curl -fsSL https://raw.githubusercontent.com/nipegun/h-scripts/main/Debian/mcOSisoCreator/mcOSisoCreator.py | python3 - --language es-AR --install-deps
```
...
Ejecutá el creador interactivo desde la carpeta del proyecto:

```sh
python3 Scripts/create_macos_iso.py --language es-AR --install-deps
```

Seleccioná una familia de macOS o escribe una versión. El programa solicita los archivos directamente a Apple, comprueba su firma RSA y todos los bloques de la imagen, lee la versión real, genera una ISO óptica y vuelve a verificar el contenido HFS+ de la ISO terminada.
...
Debian 13, Python 3.13 o posterior, `xorriso` y `7zip` actual. No necesitás paquetes de pip ni tener macOS instalado. `--install-deps` permite instalar mediante apt los paquetes que falten, usando sudo cuando corresponda. La creación normal no requiere root y nunca formatea discos ni particiones.
...
```sh
python3 Scripts/create_macos_iso.py --language es-AR
python3 Scripts/create_macos_iso.py --list --language es-AR
...
Los resultados se guardan en `ISOs/`; los temporales y las descargas reutilizables, en `_/temp/macos-iso/`. Conservá junta toda la carpeta de la herramienta, incluidos `Scripts/macos_iso`, `Sources/macos-recovery-versions.json` y `Locales/macos_iso`.
...
[Manual](MANUAL.es-AR.md) · [Referencia técnica](CODE.es-AR.md) · [English](README.md) · [Español de Argentina](README.es-AR.md)
